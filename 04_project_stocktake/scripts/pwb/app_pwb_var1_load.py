...
file_path = r'D:\Users\chironnakrit\OneDrive - Central Group\เดสก์ท็อป\Works\06_RAW_FILE\STK\STK_APP.xlsx'
...
def find_file_with_fallback(directory, filename):
    ...
    if os.path.exists(original_path):
        ...
        if os.path.exists(alt_path1):
            ...
            alt_filename2 = filename.replace("Credit", "CREDIT").replace("Consign", "CONSIGN") 
            ...
            if os.path.exists(alt_path2):
                ...
                alt_filename3 = filename.replace("Credit", "CREDIT").replace("Consign", "CONSIGN").replace(" ", "_")
                ...
                if os.path.exists(alt_path3):
                    ...
                    alt_filename4 = filename.replace("Credit", "CREDIT").replace("Consign", "CONSIGN").replace(" ", "_").replace("_20", "_")
...
for df in dataframes:
    ...
    for index, row in df.iterrows():
        ...
        consign_filename = row['filename_consign']
        credit_destination = row['path_dest_credit']
        consign_destination = row['path_dest_consign']
        ...
        # Process consign file
        found_consign_source = find_file_with_fallback(path_source,consign_filename)
        if found_consign_source and found_consign_source.endswith('.xls'):
            save_as_xlsx(found_consign_source, consign_destination)
        else:
            print(f"Consign source not found: {consign_filename}")
